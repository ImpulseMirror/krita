...
void ComfyUIRemoteDock::slotRefreshCheckpoints()
{
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, baseUrlStr]() {
        ...
        auto applyCheckpointCombo = [this](const QStringList &list) {
            ...
            if (list.isEmpty()) {
                ...
                m_d->comboCheckpoint->addItems(list);
                setStatusMessage(i18n("Loaded %1 checkpoints.", list.size()));
            }
            m_d->comboCheckpoint->setCurrentIndex(0);
        };

        if (names.isEmpty()) {
            applyCheckpointCombo(names);
            return;
        }

        // §13.75: Optional ETN model_info metadata to filter refiner-only / non-Flux inpaint / unknown arch
        const QUrl modelInfoUrl = ComfyUIUtils::comfyResolveApiUrl(baseUrlStr, QStringLiteral("api/etn/model_info/checkpoints?limit=2000"));
        QNetworkRequest reqMk(modelInfoUrl);
        ComfyUIUtils::setComfyUIRequestHeaders(reqMk);
        QNetworkReply *replyMk = m_d->nam->get(reqMk);
        connect(replyMk, &QNetworkReply::finished, this, [this, replyMk, names, applyCheckpointCombo]() {
            replyMk->deleteLater();
            QStringList finalList = names;
            if (replyMk->error() == QNetworkReply::NoError) {
                const QJsonDocument md = QJsonDocument::fromJson(replyMk->readAll());
                finalList = ComfyUIUtils::filterCheckpointNamesWithEtnModelInfo(names, md);
            }
            applyCheckpointCombo(finalList);
        });
    });
}
