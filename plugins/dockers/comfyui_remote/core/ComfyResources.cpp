...
{
    const QString m = mode.trimmed().toLower();
    return m == QLatin1String(reference) || m == QLatin1String(line_art) || m == QLatin1String(blur);
}
} // namespace ControlMode
...
QString defaultControlNetFileName(Arch arch, const QString &mode)
{
    ...
    if (m == QLatin1String(ControlMode::inpaint)) {
        ...
        if (QString f = flux("flux1-dev-controlnet-inpaint.safetensors"); !f.isEmpty())
            return f;
        if (arch == Arch::Illu || arch == Arch::IlluV)
            return QStringLiteral("noobaiinpainting");
        // Upstream resources.py: no dedicated inpaint ControlNet for SDXL — use Fooocus only.
        return QString();
    }
